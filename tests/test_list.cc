                                 ...
                                 const char *strings[])
{
    ...
    {
        (void)l->append(List::Item(*s, false, 0));
        ++expected_size;
    }
...
void test_move_down_hierarchy_without_child_list_returns_null_list(void)
{
    (void)list->append(List::Item("Foo", false, 0));
...
void test_up_and_down_one_level_of_hierarchy(void)
{
    ...
    cut_assert_equal_string("second", list->down(0)->get_item(1)->get_text());
    ...
    cut_assert_equal_string("sixth", list->down(1)->get_item(2)->get_text());
}
...
void test_up_and_down_multiple_levels_of_hierarchy(void)
{
    ...
    cut_assert_equal_uint(0, list->append(List::Item(levels[0], false, 0)));
    ...
    for(List::RamList *prev = list.get(); levels[idx] != nullptr; ++idx)
    {
        ...
        cut_assert_equal_uint(0, ram_list->append(List::Item(levels[idx],
                                                             false, 0)));
    ...
    for(const List::ListIface *l = list.get(); l != nullptr; l = l->down(0), ++idx)
    {
        ...
        const List::Item *item = l->get_item(0);
